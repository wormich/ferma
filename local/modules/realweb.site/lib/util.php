...
class Util
{
    ...
    public static function var_dump()
    {
        ...
        for ($i = 0; $i < $size; $i++) {
            $arResult = func_get_arg($i);
            self::__var_dump($arResult);
        }
    }

    public static function __var_dump($var, $return = FALSE, $expandLevel = 1)
    {

        $debug_backtrace = debug_backtrace(0, 2);
        ...
        $html = '<pre style="margin-bottom: 18px;' .
            ...
            'color: #333;' .
            'font-family: Menlo,Monaco,Consolas,\'Courier New\',monospace;">' . htmlspecialcharsbx(str_replace($_SERVER["DOCUMENT_ROOT"], '', $debug_backtrace[1]["file"]) . ":" . str_replace($_SERVER["DOCUMENT_ROOT"], '', $debug_backtrace[1]["line"])) . "</br>";;
        $done = array();
